<template>
  <main class="main">
    ...
    <div class="container__inputs">
      ...
      />
      <p class="rest__form__fieldset__field__error">
        {{ errors.confpassword }}
      </p>
      <button @click="setNewPassword" class="button__orange form__items">
        Valider
      </button>
      <p class="rest__form__fieldset__field__error">
    ...
    <div v-if="errors == 'linkExpired' || errors == 'wrongToken'">
      <p class="link__expired">
        Ce lien a expiré et/ou est faux, merci de recommencer la
        réinitialisation de votre mot de passe.
      </p>
    </div>
    <div v-if="errors == 'failChangePass'">
      <p class="link__expired">
        La réinitialisation du mot de passe n'a pas pu être éffectuée, merci de
        recommencer
      </p>
    </div>
  </main>
...
export default {
  ...
  data() {
    return {
      ...
      password: null,
      passConfirm: null,
      // On transforme la current date en timestamps
  ...
  methods: {
    async setNewPassword() {
      ...
      if (!this.passConfirm) {
        this.errors = {
          ...this.errors,
          confpassword: "La confirmation du mot de passe ne doit pas être vide",
        };
      }
      if (
        this.password &&
        ...
        this.errors = {
          ...this.errors,
          confpassword: "Les mots de passes doivent être identiques",
        };
      }

      // Si on n'a aucune erreur
      if (Object.keys(this.errors).length === 0) {
        const response = await UserService.findForResetPass(
          this.$route.query.key
        );
        if (response.code === 200) {
          if (Date.parse(response.data[1].meta_value) >= this.currentDate) {
            // Date.parse() transforme un string en milliseconde depuis le 01/01/1970
            if (response.data[0].meta_value === this.token_url) {
              ...
              if (reset_pass.code === 200) {
                ...
                this.errors = {
                  ...this.errors,
                  invalid_reset: "Le mot de passe n'a pas pu être réinitialisé",
                };
              }
            } else {
              this.errors = {
                ...this.errors,
                invalid_reset: "Le lien n'est plus valide",
              };
            }
          } else {
            this.errors = {
